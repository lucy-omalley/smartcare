...
export function PosterFlowChart({ poster, className, printMode, qrDataUrl }: PosterFlowChartProps) {
  ...
  return (
    ...
    >
      <div className={cn("flex flex-col h-full", isCompact ? "p-4" : "p-6 md:p-8")}>
        <header className="text-center space-y-1 mb-4">
          <p className="text-3xl">{theme.emoji}</p>
          <h2
            className={cn(theme.fontClass, isCompact ? "text-lg" : "text-2xl")}
        ...
        <div className="flex-1 flex flex-col items-center gap-1 min-h-0 overflow-y-auto">
          {poster.steps.map((step, i) => (
            <div key={step.id} className="flex flex-col items-center w-full">
              ...
              >
                <span className={cn(isCompact ? "text-3xl" : "text-5xl")} role="img" aria-hidden>
                  {step.iconEmoji}
                </span>
                <p
                  className={cn(theme.fontClass, "flex-1 text-left", isCompact ? "text-sm" : "text-lg")}
              ...
              {i < poster.steps.length - 1 && (
                <span
                  className={cn("my-0.5", isCompact ? "text-xl" : "text-2xl")}
